...
parser.add_argument('--resolution', type=int, choices=[128, 256], required=True, help="Resolution size.")
...
data_path = f'ot-data/LDC_NS_2D_{resolution}_{group_name}_{args.latent_shape}_expand'+str(expand_factor) +'_reg1e-6.pt'
data = torch.load(data_path)
origin_data = np.load('/data/xinyili/datasets/flowbench/LDC_NS_2D/' + str(resolution) + 'x' + str(resolution) + '/' + group_name + '_lid_driven_cavity_X.npz')
print(data_path)
...
train_trans_points = data['trans_points'][0:n_train]
train_trans_sdfs = data['trans_sdfs'][0:n_train]
train_inputs = [torch.cat([generate_latent_grid(train_trans_points[i].shape[0]), train_trans_points[i], train_trans_sdfs[i], torch.ones_like(train_trans_sdfs[i])*origin_data['data'][i,0,0,0], torch.ones_like(train_trans_sdfs[i])*origin_data['data'][i,2,0,0]], dim=2) for i in range(n_train)]
...
test_trans_points = data['trans_points'][n_train:]
test_trans_sdfs = data['trans_sdfs'][n_train:]
test_inputs = [torch.cat([generate_latent_grid(test_trans_points[i].shape[0]), test_trans_points[i], test_trans_sdfs[i], torch.ones_like(test_trans_sdfs[i])*origin_data['data'][i+n_train,0,0,0], torch.ones_like(test_trans_sdfs[i])*origin_data['data'][i+n_train,2,0,0]], dim=2) for i in range(n_test)]
...
#train_dict['pos'] = [rectangular_grid(train_dict['inputs'][i].shape[0]) for i in range(n_train)]
#test_dict['pos'] = [rectangular_grid(test_dict['inputs'][i].shape[0]) for i in range(n_test)]

train_dataset = DictDataset(train_dict)
test_dataset = DictDataset(test_dict)

#train_dataset = DictDatasetWithConstant(train_dict, {'pos': pos.to(dtype=torch.float32), 'nor':nor.to(dtype=torch.float32)})
#test_dataset = DictDatasetWithConstant(test_dict, {'pos': pos.to(dtype=torch.float32), 'nor':nor.to(dtype=torch.float32)})
