...
class PyTorchStochasticDiscreteOT(PyTorchStochasticOT):
    ...
    def __init__(self, xs=None, ws=None, xt=None, wt=None, reg_type='entropy', reg_val=0.1, device_type='cpu', device_index=0):
        ...
        self.Xs = torch.from_numpy(xs).float().to(device=self.device)
        #self.ws = torch.from_numpy(ws).to(device=self.device)
        self.ws = ws
        self.Xt = torch.from_numpy(xt).float().to(device=self.device)
    ...
    def learn_OT_dual_variables(self, epochs=10, batch_size=100, batch_number_per_epoch=100, optimizer=None, lr=0.01):
        ...
        for e in range(epochs):
            ...
            print(f'Epoch : {e+1}, loss = {np.sum(batch_losses)}, time = {self.time[-1]-self.time[-2]:.2f}')
    ...
    def learn_barycentric_mapping(self, neuralNet=None, epochs=10, batch_size=100, batch_number_per_epoch=100, optimizer=None, lr=0.01):
        ...
        for e in range(epochs):
            ...
            for b in range(batch_number_per_epoch):
                ...
                i_s = torch.from_numpy(np.random.choice(self.ns, size=(batch_size,), replace=False, p=self.ws)).type(torch.LongTensor).to(device=self.device)
                #i_s = torch.from_numpy(np.random.choice(self.ns, size=(batch_size,), replace=False, p=self.ws.cpu().numpy())).type(torch.LongTensor).to(device=self.device)
                i_t = torch.from_numpy(np.random.choice(self.nt, size=(batch_size,), replace=False, p=self.wt)).type(torch.LongTensor).to(device=self.device)
                #i_t = torch.from_numpy(np.random.choice(self.nt, size=(batch_size,), replace=False, p=self.wt.cpu().numpy())).type(torch.LongTensor).to(device=self.device)
            ...
            print(f'Epoch : {e+1}, barycentric_mapping loss = {np.sum(batch_losses)}, time = {barycentric_mapping_time[-1]-barycentric_mapping_time[-2]:.2f}')
